# MURU v2 — CALIBRATION / RE-ENTRY PROTOCOL, **VERSION 2**
...
No world generated. No module written. No search executed. No re-entry licensed.
...
### 0.2 DECISION 2 — Route C+D → E4f is now EXECUTABLE
...
| `MURU_V2_E4F_OPERATIONAL_PREREGISTRATION.md` @ `8a2ffa50` | the operational freeze that makes the `C+D` route executable (Decision 2) |
...
tuple constructed at runtime in the new module. **`DECLARED_BANDS` is not mutated**, because
...
`delta` against the precision clause* — with only the critical value corrected. Using the
...
| **C-6a — downstream-freeze integrity** | `MURU_V2_E4F_OPERATIONAL_PREREGISTRATION.md` still hashes to `0ce2755d…3a7f61` and `8a2ffa50` is still a strict ancestor of HEAD; `E4F_FREEZE.txt`'s tuning ledger is still empty. Checked **before** Gate R is read, so a downstream freeze drift can never present as a routing outcome | binary |
...
Note the interaction with `P6'`: because `INDETERMINATE_WORLDS == 0` is a hard precondition,
`rho_bot` and `rho_top` **coincide** on any admissible surface, so the "both resolutions"
clauses are mechanically satisfiable and the argmax-invariance clause is a genuine check that
fails loudly rather than a formality that cannot fail.
...
| 3 | `ROUTING_CERTIFIED` and certified argmax = `C+D` (`LOST_IN_CROSS_SEED`) | RC7 → **E4f** (classifier / voting) | **YES — this is Decision 2.** `MURU_V2_E4F_OPERATIONAL_PREREGISTRATION.md` @ `8a2ffa50` is a complete operational freeze: population, DEV/EVAL split, acceptance predicate, both previously-absent ceilings at margin exactly 0, identity/replay control, terminal set, threshold inventory, free-parameter list |
...
requires none, and because it **dominates**: `pi_B < delta` implies
`P_retain_given_front = S_2/S_1 >= 1 - delta/S_1 >= 1 - delta` whenever `S_1 <= 1`, so any
surface satisfying the absolute form satisfies "`P_retain_given_front` within `delta` of 1"
unconditionally.
...
1. **The §21.4 annotation in full**, including its interval. If it reads `CONTRADICTS`, the
   record must additionally contain the owner's **written explanation** of the divergence.
   This is the operationalisation of `befca0d` §2.3's *"blocks adoption ... until explained"*,
   and it is the reason removing Gate V does not remove that clause's force. It is not a
   veto: it is dischargeable, it applies identically to all three routes, and the discharge
   is an owner act on the record rather than an arithmetic threshold.
...
1. **The E6 false-structure ceiling is a precondition, not a report — and the circularity v1
   could not resolve is dissolved** (`D13(3)`, `S14`). v1 wrote that *"E6 is self-blocked
   ...
   *"100 evaluable safety opportunities … unsafe acceptance Wilson upper <= 0.15 => change
   ...
   which clears the frozen `>= 100` bar by a factor of 2.76 without executing E6 at all. An
   arm that recovers cases and breaches `Wilson upper <= 0.15` on that stratum **is not
   licensed**. No E6 execution is required and none is presumed.
...
| F2 | `Q1` or `P9` fails for a mechanical reason (ordinal drift, seed-band collision, `GENERATOR_VERSION` mismatch, `pb_33`/`pb_34` non-zero, unauthorised protected-path drift) | `BENCHMARK_INTEGRITY_DEFECT` |
...
| F10 | `QUALIFIED` and Gate R row 1 | `E4A_LICENCE_PROPOSED_AT_<arm>` |
| F11 | `QUALIFIED` and Gate R row 2 | `E4_GENERATION_LICENCE_PROPOSED_F09_F10` |
| F12 | `QUALIFIED` and Gate R row 3 | `E4F_LICENCE_PROPOSED` |
| F13 | Any of D3's eight `EXPERIMENTAL_REENTRY_RESOLUTION` items unmet at verdict time, or the §21.5 owner ratification is refused or not produced | `D3_ITEMS_UNMET_NO_REENTRY` |
| F14 | The protocol owner concludes that `befca0d` §2.3 combined with D6 admits no qualification that is both non-circular and non-vacuous | `T1_NO_ADMISSIBLE_QUALIFICATION_EXISTS` |
...
| `D-INST-INDETERMINATE` | ≥1 affected E2a world's stage is not invariant; the interval is reported | **`T-INSTRUMENT-UNBOUNDED-ON-E2A`. Stage 1 forbidden** |
...
| `E4F_LICENCE_PROPOSED` | F12 | Certified route `C+D`. **Executable — Decision 2.** E4f's own terminals then govern (`E4F_I_*`, `E4F_II_*`), none of which grants a licence either; E4f's outputs are efficacy-admissibility statements pending E6 | **Yes** |
...
| `276 = 2 x 138` | NEG control worlds | **§5.2.** Same replicate count on the two negative-control families; 2.76× E6's frozen `>= 100 evaluable safety opportunities` |
