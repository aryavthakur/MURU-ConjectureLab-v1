...
**Not performed on either candidate source; both validation populations correctly preserved.** The final-candidate freeze (Part I, commit `24a4ca1`) fixed the candidate, comparators, populations, endpoint, adapter families, compatibility gate and success rule before any MultiMS2 peak was decoded. The anchor calibration (106 identity-exposed compounds, access record at HEAD `6c6cc4f`, clean tree) failed the gate: best adapter Spearman 0.33/0.66/0.72 and median absolute difference 0.078/0.074/0.060 at 20/40/60 eV, pooled RMSD 0.13 (required 0.80, 0.05, 0.08); the two-parameter adapter also failed. Model-free diagnostics confirm a real incompatibility rather than a defect: even the best rank correlation between MultiMS2 mu and a compound's exposed Orbitrap trajectory at any rung is 0.47 at 20 eV and 0.56 at 40 eV. The frozen rule therefore stopped the study: the 1,297-compound validation population and the 228-compound secondary population remain outcome-unaccessed, and **MultiMS2 is not qualified for the desired external claim**.

**Second source: MSnLib** (`MURU_V2_MSNLIB_OUTCOME_BLIND_CENSUS.md`, `MURU_WUR_V2_MSNLIB_EXTERNAL_PROTOCOL.md`, freeze Parts III and IV). Orbitrap ID-X Tribrid, flow injection of 8 to 10 compound wells, HCD fixed NCE 20 and 60 per precursor plus an outcome-adaptive Assisted scan that is never used; first mass 40 m/z as in development. Census design frame after identity, overlap, scaffold-new, charge-neutral scaffold, same-well conflict and mass-range rules: 39,238 compounds in 29,562 groups; at most a **two-rung fixed-NCE transfer claim**. With the user's anchors-first authorization, 561 anchor wells (1.26 GB) were downloaded, a two-rung protocol was frozen (Part III) and the anchor gate was run on 328 anchors (a first attempt stopped before decoding any value because the files use MS-Numpress compression; the reader was fixed and tested, and the attempt is recorded). **The gate failed by very small margins**: the zero-parameter frozen WUR map gave NCE 20 median absolute difference 0.0501 against a limit of 0.05 (Spearman 0.88 and 0.90, RMSD 0.078); the one-parameter adapter passed both medians but had RMSD 0.0811 against 0.08; the two-parameter adapter missed the NCE 20 median (0.0506). The frozen rule was applied literally: **MSnLib is not qualified under the frozen gate**, and its validation population was not sampled, downloaded or decoded. Descriptively, the MSnLib observable is well aligned with the development coordinate (model-free Spearman 0.94 and 0.96 at matched energies), so a separately pre-registered MSnLib study with the zero-parameter map fixed in advance is the most promising route to a first external test of v2; a prospective dense-NCE acquisition remains the strongest.
