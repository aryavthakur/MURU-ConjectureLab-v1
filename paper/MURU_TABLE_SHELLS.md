...
case-shaped frames) are frozen under Amendment A3.4. The acceptance predicate's
gate 7 and gate 8 rows are frozen under Amendment A3.5
...
| 8 | falsification harness (A3.5) | F1, F4, F7, F10 all `PASS`, fail-closed: only an explicit `PASS` passes, so a missing rung, a `FAIL`, or a `NOT_APPLICABLE` no rung may emit all reject identically. F5 is superseded (its waiver-regime floor is folded into gate 7); F9 is computed and reported, never gating; F8 is labelling, not a gate | not accepted |
