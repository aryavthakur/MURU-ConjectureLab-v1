...
**PRE-RESULTS DRAFT.** Synchronized through Amendment A3.5, before the
current-contract Development symbolic search was run, and while Held-out,
Challenge and Confirmation remained strictly sealed, unconstructed or unopened.
...
| **CLASS B** | Frozen prospective methods (benchmark architecture, A1/A2/A2.1/A3.1/A3.2/A3.3/A3.4/A3.5, Engineering RC3/RC3.1/RC4/RC4.1/RC4.2/RC4.2.1/RC5, A3.4 temporal provenance erratum, endpoints, thresholds, provenance rules) | Yes |
...
Governance base of this draft: **Amendment A3.5**
(`560bf28568e2762c60edc994aac7f2b6de14081f`, tag `benchmark-content-freeze-a3-5`,
annotated tag object `533777b73748e3c45dd1ecbda07098ba9837c587`), following
Amendment A3.4 (`be23b80d63fbd30227f0ab8f200dddc2121f3bfe`, tag
`benchmark-content-freeze-a3-4`), Amendment A3.3
(`71f53697e8894df6469ad0ff7150a049fa531b74`, tag `benchmark-content-freeze-a3-3`),
the A3.3 hostile mathematical review (`78cc7c2`), the A3.4 pre-engineering
integrity review (`f1fb943`), and the A3.4 Temporal Provenance Erratum
(`220c9cb679e03865f1b2a02b975397de9f4c7b46`, tag
`a3-4-temporal-provenance-erratum`).

A3.5's engineering parent is RC4.2.1
(`69e33c778efb14362439941d25ebbfcfb1068284`, tag
`engineering-rc4-2-1-integrity-closure`); the executable implementation of A3.5
is Engineering RC5 on branch `eng/muru-rc5-a3-5`. The A3.5 science lineage and
the engineering lineage are siblings, not one merged history.
...
recorded amendments (A1, A2, A2.1, A3.1, A3.2, A3.3, A3.4, the A3.4
temporal provenance erratum, and A3.5), each of which declares its temporal position with
...
support, a gradient-boosted ceiling condition whose low-ceiling waiver requires
the candidate's own test R2 to clear the same null threshold, and a
fail-closed falsification harness of four hard rungs. The acceptance threshold is not assumed; it is calibrated on 100
...
**The falsification harness (Amendment A3.5).** Structural acceptance requires
exactly four hard rungs to pass: **F1 reproducibility, F4 compound holdout, F7
influence-drop, F10 negative control**. The check is **fail-closed**: only an
explicit `PASS` passes, so a missing rung, a `FAIL`, and a `NOT_APPLICABLE`
that no rung is permitted to emit all reject identically
(`MURU_PAPER_BENCHMARK_AMENDMENT_A3_5.md` 6.9.4).

Two rungs of the earlier six-rung design changed role under A3.5, and neither
change is silent:

- **F5 scaffold holdout is superseded** as an independent rung. It is
  structurally redundant with F7 in the ordinary case, since F7's exhaustive
  leave-one-scaffold-out statistic is computed on the identical test rows under
  training sets that are strict subsets of F5's own. Its one non-redundant
  contribution -- a floor on the candidate's own test R2 in the low-ceiling
  waiver regime -- is folded into the ceiling gate, where the waiver now
  requires both `ceiling_r2 < 0.05` **and** `candidate_test_r2 >
  null_threshold[min(complexity, 20)]` (6.9.2, 6.9.3). No new threshold,
  constant or calibration object was introduced by the fold: it reuses the
  already-computed null threshold table and the already-bound
  `candidate_test_r2` quantity, computed once and shared with the ceiling
  estimate.
- **F9 energy-subset stability is reported, not gating.** It is computed for
  every case reaching the harness, exactly as specified, and recorded as
  `f9_stress_test_result` and `f9_stress_test_metric`; it can never by itself
  produce a rejection. Its acceptance calibration status is
  `NOT_PROVEN_FOR_HARD_GATE`: F9 is the only rung that re-estimates the
  scalar/`g` pipeline inside each fold, a strictly larger perturbation than the
  two-parameter affine refit the null threshold's conservatism argument was
  derived for, so the threshold's direction of bias for F9's statistic is
  unestablished in either direction. **No claim in this manuscript cites F9's
  PASS/FAIL as evidence of validated robustness** (6.9.4).

F8 is structural labelling and is explicitly **not** an acceptance gate.

Two further namings are binding and are used consistently below. F1 tests
**execution determinism**, not robustness to seed choice and not discovery
reproducibility -- seed-robustness is measured separately by the 20-of-30
stability gate. F4 is a **coefficient-generalisation** rung, not a discovery
holdout: the structural search fit on the full training partition, so it had
already seen every compound F4 evaluates. F10 is a **pipeline-integrity
control**, not a statistical significance test (`MURU_PAPER_BENCHMARK_AMENDMENT_A3_5.md`
6.1, 6.2, 6.6).
...
hard-rung failures by rung (F1, F4, F7, F10), reported separately from the
non-gating F9 stress-test result. Under Amendment A3.5 a case that never
reached the harness carries no rung result at all, and that state must be
distinguished in the census from a rung that ran and failed. The per-endpoint
count of execution-failure-poisoned cases is a mandatory separate disclosure
(`MURU_PAPER_BENCHMARK_AMENDMENT_A3_5.md` 8.2). Status:
`[PROSPECTIVE RESULT TO INSERT]`.
